...
const activationData: Record<
  string,
  { method: string; slot: string; maxCashout: string; expiry: string }
> = {
  'iwild-casino': { method: 'Automat la depunere', slot: 'Diverse sloturi', maxCashout: '—', expiry: '7 zile' },
  'velobet-casino': { method: 'Automat la depunere', slot: 'Diverse sloturi', maxCashout: '—', expiry: '14 zile' },
  'billionairespin-casino': { method: 'Automat la depunere', slot: 'Diverse sloturi', maxCashout: '—', expiry: '7 zile' },
  'fortunejack-casino': { method: 'Automat la depunere', slot: 'Diverse sloturi', maxCashout: 'Nelimitat', expiry: '30 zile' },
  'spinbetter-casino': { method: 'Automat la depunere', slot: 'Diverse sloturi', maxCashout: '—', expiry: '7 zile' },
  'rolletto-casino': { method: 'Automat la depunere', slot: 'Book of Dead', maxCashout: '100€', expiry: '14 zile' },
  'flagman-casino': { method: 'Automat la depunere', slot: 'Diverse sloturi', maxCashout: '—', expiry: '14 zile' },
  'gamblezen-casino': { method: 'Automat la depunere', slot: 'Diverse sloturi', maxCashout: '—', expiry: '14 zile' },
  'immerion-casino': { method: 'Automat la depunere', slot: 'Diverse sloturi', maxCashout: '—', expiry: '14 zile' },
  'xon-bet-casino': { method: 'Automat la depunere', slot: 'Diverse sloturi', maxCashout: '—', expiry: '14 zile' },
};

export function ActivationGuideSection() {
  const casinos = getCasinosBySlugs(rotiriGratuiteSlugs).slice(0, 10);
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="mb-10 text-center">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold font-mono text-gray-900">
            Cum Activezi Rotirile Gratuite la Fiecare Cazinou
          </h2>
          <p className="text-base text-gray-500 mt-2 max-w-2xl mx-auto">
            Detalii practice per cazinou: metodă de activare, slot eligibil, rulaj, câștig maxim și termen
          </p>
        </div>
        ...
        <div className="hidden lg:block rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <table className="w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {['Casino', 'Rotiri', 'Slot Eligibil', 'Activare', 'Rulaj', 'Max Câștig', 'Expirare', ''].map(
                  (h) => (
                    <th
                      key={h}
            ...
            return (
              ...
              >
                ...
                <p className="text-xs font-semibold text-orange-600 font-mono mb-3">
                  {casino.freeSpins} rotiri gratuite
                </p>
                <div className="grid grid-cols-2 gap-2 text-center mb-3">
                  ...
                  <div className="rounded-lg bg-gray-50 py-1.5 px-1">
                    <p className="text-[10px] text-gray-500 font-mono uppercase">Slot</p>
                    <p className="text-xs font-bold font-mono text-gray-900">{data?.slot ?? '—'}</p>
                  </div>
                  <div className="rounded-lg bg-gray-50 py-1.5 px-1">
