    ...
    <>
      ...
      <div className="lg:hidden relative py-2">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-48 h-32 bg-gradient-to-r from-amber-400/30 via-orange-500/30 to-amber-400/30 rounded-full blur-3xl" />
        </div>
        ...
        <div className="relative flex items-center justify-center gap-2 mb-3">
          ...
          <span className="text-xs font-black uppercase tracking-widest text-amber-600">Top 3</span>
          <Trophy className="w-5 h-5 text-amber-500 fill-amber-500" />
        ...
        <div className="relative flex items-end justify-center gap-1.5">
          {podiumOrder.map((casino, idx) => {
            ...
            const gradients = {
              1: 'from-amber-400 via-yellow-500 to-orange-500',
              2: 'from-slate-400 via-slate-500 to-slate-600',
              3: 'from-amber-600 via-amber-700 to-orange-700',
            };

            const shadows = {
              1: 'shadow-amber-500/50',
                ...
                >
                  ...
                  <div className="relative px-2 pt-4 pb-4">
                    ...
                    <div className={`mt-4 rounded-lg text-center transition-all ${
                      isFirst
                        ? 'bg-white text-amber-600 py-2'
                        : 'bg-white/25 text-white py-1.5'
                    }`}>
                      <span className={`font-bold flex items-center justify-center gap-0.5 ${
                        isFirst ? 'text-[10px]' : 'text-[8px]'
                      }`}>
                        JOACĂ <ExternalLink className={isFirst ? 'w-3 h-3' : 'w-2.5 h-2.5'} />
                      </span>
                    </div>
                  </div>
                </div>

                {/* Podium base with padding */}
                <div className={`rounded-b-lg ${
                  isFirst
                    ? 'h-8 bg-gradient-to-b from-orange-500 to-orange-700'
                    : isSecond
                      ? 'h-6 bg-gradient-to-b from-slate-500 to-slate-700'
                      : 'h-5 bg-gradient-to-b from-amber-700 to-amber-900'
                }`}>
              ...
              >
                ...
                <div className={`relative overflow-hidden rounded-t-3xl shadow-2xl transition-all duration-300 ${
                  ...
                  <div className={`absolute top-3 right-3 z-20 flex items-center justify-center rounded-full font-black text-sm ${
                    isFirst
                      ? 'w-8 h-8 bg-white text-amber-600 shadow-lg'
                  ...
                  <div className={`relative p-4 ${isFirst ? 'pt-12' : 'pt-10'}`}>
                    ...
                    {isFirst && (
                      <div className="mt-3 bg-white/20 backdrop-blur-sm rounded-xl px-3 py-2">
                        <p className="text-white/80 text-[10px] font-semibold uppercase tracking-wide text-center">Bonus</p>
                    ...
                    <div className={`mt-3 rounded-xl text-center transition-all duration-300 ${
                      isFirst
                        ? 'bg-white text-amber-600 py-2.5 group-hover:bg-amber-50 shadow-lg'
                        : 'bg-white/20 text-white py-2 group-hover:bg-white/30'
                    }`}>
                      <span className={`font-bold flex items-center justify-center gap-1 ${
                        isFirst ? 'text-sm' : 'text-xs'
                      }`}>
                        JOACĂ <ExternalLink className={isFirst ? 'w-3.5 h-3.5' : 'w-2.5 h-2.5'} />
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className={`relative overflow-hidden rounded-b-2xl ${
                  isFirst
                    ? 'h-16 bg-gradient-to-b from-orange-500 to-orange-700'
                    : isSecond
  ...
  return (
    <>
      ...
      <div className="lg:hidden relative py-2">
        ...
        {/* Horizontal cards with podium heights */}
        <div className="relative flex items-end justify-center gap-2 px-1">
          {podiumOrder.map((casino, idx) => {
            const isFirst = idx === 1;
            const isSecond = idx === 0;
            const position = isFirst ? 1 : isSecond ? 2 : 3;
            ...
            return (
              <a
                ...
                rel="noopener noreferrer sponsored"
                className={`group relative flex-1 transition-all duration-300 active:scale-95 ${
                  isFirst ? 'max-w-[140px] z-10' : 'max-w-[105px]'
                }`}
              >
                {/* Main card */}
                <div
            ...
            return (
              ...
              >
                ...
                <div className={`relative overflow-hidden rounded-t-3xl shadow-2xl transition-all duration-300 ${
                  ...
                  {/* Position badge */}
                  <div className={`absolute top-3 right-3 z-20 flex items-center justify-center font-black text-sm ${
                    isFirst
                      ? 'w-8 h-8 bg-white text-amber-600 shadow-lg'
                      : 'w-7 h-7 bg-white/20 text-white backdrop-blur-sm'
                  }`}>
                  ...
                  <div className={`relative p-4 ${isFirst ? 'pt-12' : 'pt-10'}`}>
                    ...
                    {/* Bonus - show on all */}
                    <div className="mt-2 bg-white/20 backdrop-blur-sm rounded-xl px-3 py-2">
                      <p className={`text-white font-bold text-center leading-tight line-clamp-2 ${
